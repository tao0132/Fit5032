...
const submitForm = () => {
  ...
  if (!errors.value.username && !errors.value.password ) 
  {
    ...
    if (formData.value.username === 'admin' && formData.value.password === 'Aa12345678!'){
      ...
      .then((data) => {
        console.log("Firebase Login Successful data", data)
        console.log("Firebase Login Successful!", getAuth().currentUser)
        isAuthenticated.value = true
        router.push("About")
      })
      .catch((error) => {
        console.log("Firebase Login Error:", error.code, error.message)
        errors.value.authentication = `Login failed: ${error.message}`
      })
    }
    else{
  ...
  <div class="container mt-5">
    <div class="row">
      <div class="col-md-8 offset-md-2">
        <h1 class="text-center">Fireabse  Log in</h1>
        <!-- <p class="text-center">
